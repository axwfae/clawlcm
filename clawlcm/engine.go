...
const (
	defaultAvgDocLen     = 50.0
	defaultMsgLimit      = 1000
	defaultAssembleLimit = 10000
	largeMsgLimit        = 100000
)
...
func (e *Engine) Bootstrap(ctx context.Context, req types.BootstrapRequest) (*types.BootstrapResponse, error) {
	...
	for _, msg := range req.Messages {
		...
		msgID, err := e.store.CreateMessage(conv.ID, ordinal, msg.Role, msg.Content, tokens)
		if err != nil {
			return nil, fmt.Errorf("create message: %w", err)
		}
		keywords := tk.ExtractKeywords(msg.Content, 20)
		if keywords != "" {
			e.store.CreateContextItem(conv.ID, types.ContextItemMessage, msgID, ordinal, tokens, keywords)
		}
		ordinal++
		totalTokens += tokens
	}
	...
	messages, err := e.store.GetMessages(conv.ID, defaultMsgLimit, 0)
	if err != nil {
...
func (e *Engine) Ingest(ctx context.Context, req types.IngestRequest) (*types.IngestResponse, error) {
	...
	// Create context item with keywords for BM25 search
	tk := tokenizer.New()
	keywords := tk.ExtractKeywords(req.Message.Content, 20)
	if keywords != "" {
		e.store.CreateContextItem(conv.ID, types.ContextItemMessage, msgID, count, tokens, keywords)
	}
...
func (e *Engine) Assemble(ctx context.Context, req types.AssembleRequest) (*types.AssembleResponse, error) {
	...
	messages, err := e.store.GetMessages(conv.ID, defaultAssembleLimit, 0)
	if err != nil {
		return nil, fmt.Errorf("get messages: %w", err)
	}
...
func (e *Engine) Compact(ctx context.Context, req types.CompactRequest) (*types.CompactResponse, error) {
	...
	// Force override when explicit maxDepth is set
	forceOverride := req.MaxDepth > 0

	if compactionMode == "deferred" && !req.Force && !forceOverride {
	...
	// Use request maxDepth if provided, otherwise use calculated depth
	effectiveMaxDepth := maxDepth
	if req.MaxDepth > 0 {
		effectiveMaxDepth = req.MaxDepth
	}

	if e.cfg.IncrementalMaxDepth >= 0 && effectiveMaxDepth >= e.cfg.IncrementalMaxDepth && req.MaxDepth == 0 {
	...
	if effectiveMaxDepth == 0 {
		...
		// When requesting condensed (maxDepth > 0), get summaries at the depth below
		// If we have Leaf summaries (depth=0), compress them to depth 1
		sourceDepth := effectiveMaxDepth - 1
		if sourceDepth < 0 {
			sourceDepth = 0
		}

		depthSummaries, err := e.store.GetSummariesByDepth(conv.ID, sourceDepth)
		if err != nil {
...
func (e *Engine) maintainGC(resp *types.MaintainResponse) error {
	...
	for _, conv := range convs {
		...
		for _, sum := range summaries {
			...
			if !isReferenced && sum.Depth > 0 {
				bytesFreed += int64(len(sum.Content))
				e.log.Info(fmt.Sprintf("GC: removing orphaned summary %d (depth=%d, tokens=%d)",
					sum.ID, sum.Depth, sum.TokenCount))
			}
		}
	}
...
func (e *Engine) maintainBackup(resp *types.MaintainResponse) error {
	...
	backupPath := filepath.Join(filepath.Dir(e.cfg.DatabasePath), fmt.Sprintf("backup_%s.db", timestamp))

	// Create actual backup by copying the database file
	if err := copyFile(e.cfg.DatabasePath, backupPath); err != nil {
		return fmt.Errorf("failed to create backup: %w", err)
	}

	// Also backup WAL and SHM files if they exist
	walPath := e.cfg.DatabasePath + "-wal"
	shmPath := e.cfg.DatabasePath + "-shm"

	if _, err := os.Stat(walPath); err == nil {
		walBackup := backupPath + "-wal"
		if err := copyFile(walPath, walBackup); err != nil {
			e.log.Warn(fmt.Sprintf("failed to backup WAL file: %v", err))
		}
	}
	...
	if _, err := os.Stat(shmPath); err == nil {
		...
		if err := copyFile(shmPath, shmBackup); err != nil {
			e.log.Warn(fmt.Sprintf("failed to backup SHM file: %v", err))
		}
	}
	...
	for _, conv := range convs {
		...
		for _, sum := range summaries {
			if sum.ParentIDs != nil {
				for _, pid := range sum.ParentIDs {
					if !validParents[pid] {
						if err := e.store.DeleteSummary(pid); err == nil {
							cleaned++
							e.log.Warn(fmt.Sprintf("removed orphaned parent summary %d", pid))
						}
					}
				}
			}
...
func (e *Engine) maintainRotate(sessionKey *string, resp *types.MaintainResponse) error {
	...
	messages, err := e.store.GetMessages(conv.ID, largeMsgLimit, 0)
	if err != nil {
	...
	e.log.Info(fmt.Sprintf("Rotate: kept %d messages, cleared %d, summaries cleared", keepCount, msgCount-keepCount))
	return nil
}
...
func (e *Engine) Grep(ctx context.Context, req types.GrepRequest) (*types.GrepResponse, error) {
	...
	if req.Mode == "regex" && req.Pattern != "" {
		...
		if err != nil {
			return &types.GrepResponse{Success: false, Error: fmt.Sprintf("invalid regex pattern: %v", err)}, nil
		}
	}

	for _, convID := range convIDs {
		if scope == "messages" || scope == "all" {
			msgs, _ := e.store.GetMessages(convID, defaultMsgLimit, 0)
			for _, m := range msgs {
				score := 0.0
				content := m.Content
				if regex != nil {
