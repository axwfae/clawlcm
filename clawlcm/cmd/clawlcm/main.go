...
const (
	version           = "0.8.9"
	commit            = "fix-help-version-and-keywords"
	defaultLLMTimeout = 120000
)
...
func getDefaultBasePath() string {
	...
	return filepath.Join(parentDir, "data")
}
...
func createDefaultConfig(path string) error {
	...
	defaultConfig.Database.Path = "clawlcm.db"
	defaultConfig.LLM.Provider = "openai"
	defaultConfig.LLM.TimeoutMs = defaultLLMTimeout
...
func loadConfig(path string) (*ConfigFile, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			data, err = os.ReadFile(path)
			if err != nil {
				return nil, fmt.Errorf("read newly created config %s: %w", path, err)
			}
		} else {
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "clawlcm - Lossless Context Management v%s\n\n", version)
	...
	if cmd == "bootstrap" || cmd == "ingest" {
		...
		for i, arg := range remainingArgs {
			...
			if (arg == "--mode" || arg == "-mode") && i+1 < len(remainingArgs) && cmd == "grep" {
				*mode = remainingArgs[i+1]
			}
			if strings.HasPrefix(arg, "--mode=") && cmd == "grep" {
	...
	cfgPath := filepath.Join(getDefaultBasePath(), "config.json")
	if *configFile != "" {
		cfgPath = *configFile
	}
	...
	if *dbPath != "" {
		...
		cfg.DatabasePath = resolvePath(cfgJSON.Database.Path, basePath)
	} else {
		cfg.DatabasePath = filepath.Join(basePath, "clawlcm.db")
	}
	...
	if cfgJSON.Context.LargeFilesDir != "" {
		cfg.LargeFilesDir = resolvePath(cfgJSON.Context.LargeFilesDir, basePath)
	} else {
		cfg.LargeFilesDir = filepath.Join(basePath, "large_files")
	}
...
func handleGrep(ctx context.Context, engine *clawlcm.Engine, sessionKey, pattern, mode, scope *string, allConv *bool, limit *int, sort *string) {
	...
	if !resp.Success {
		fmt.Fprintf(os.Stderr, "Grep failed: %s\n", resp.Error)
		os.Exit(1)
	}
