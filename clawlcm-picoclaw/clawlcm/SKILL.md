...
metadata: {nanobot:{emoji:🧠,requires:{bins:[clawlcm]},install:[{id:manual,kind:binary,label:將 clawlcm 二進制複製到 skill bin 目錄}]}}
---
...
在使用 clawlcm 之前，**必須先配置 LLM 相關設置**於 `config.json`，否則無法正常運行！
...
> **⚡ 路徑說明**：數據庫和配置檔路徑均為 **相對路徑**，基於執行命令時的當前工作目錄 (CWD)。
> 建議使用絕對路徑或確保在正確的目錄執行：
> ```bash
> # 使用絕對路徑
...
| `context.largeFilesDir` | `./data/large_files` | **⭐ 新增** 大文件目錄 |
...
記錄本技能測試中發現的二進制文件 bug：

### 1. Describe/Expand CLI 參數解析問題 ✅ 已修復

**修復內容**：為 describe 和 expand 命令添加了剩餘參數解析支援。

**正確用法**：
```bash
# describe - 需要 session-key 作為第一個參數
describe --session-key session_key --id 摘要ID
describe -session-key session_key -id 摘要ID

# expand - 需要 session-key 作為第一個參數
expand --session-key session_key --summary-ids id1,id2
expand -session-key session_key --summary-ids 1 --query "查詢" --max-depth 3
```

**狀態**：✅ 已修復 (v0.8.2 cli-fix)

### 2. Maintain -op 參數被忽略 ✅ 已修復

**修復內容**：為 maintain 命令添加了 `-op`、`-maint-op` 等多種參數格式支援。

**正確用法**：
```bash
maintain -op gc                  # 單破折號 + 空格
maintain --op=vacuum           # 雙破折號 + 等號
maintain -op=backup           # 單破折號 + 等號
maintain -maint-op doctor       # 完整參數名
maintain --maint-op=clean     # 完整參數名 + 等號
```

**狀態**：✅ 已修復 (v0.8.2 cli-fix)

### 3. Compact 需要 LLM 配置但無效

**症狀**：即使通過命令行傳入 LLM 配置，Compact 仍回報 "summarizer not configured"。
...
**注意**：這可能是預期行為（需要有效配置），但錯誤訊息不夠清晰。

### 4. Grep/Assemble -all 參數行為不一致

**症狀**：`assemble -all` 要求 `--session-key`，但邏輯上 `-all` 應該忽略 session-key。

**測試記錄**：
```bash
$ ./bin/clawlcm assemble -all
Error: --session-key is required     # ⚠️ 邏輯衝突

$ ./bin/clawlcm grep -all        # 正常運作
$ ./bin/clawlcm grep --all     # 正常運作
```

### 5. Ingest --messages 參數行為

**症狀**：`--messages` JSON 格式不被識別，需要與 `--content` 配合使用。
...
### 3. Compact summarizer 配置需求 (已解決)

**說明**：Compact 現在可以在 config.json 中配置 LLM 設置後正常運行。
