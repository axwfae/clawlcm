...
metadata: {nanobot:{emoji:🧠,requires:{bins:[clawlcm]},install:[{id:manual,kind:binary,label:Copy clawlcm binary to skill bin directory}]}}
---
...
Before using clawlcm, **you must configure LLM settings** in `config.json`, otherwise it won't work!
...
> **⚠️ Path Note**: Database and config paths are **relative paths**, based on the current working directory (CWD) when executing commands.
...
| `context.largeFilesDir` | `./data/large_files` | **⭐ NEW** Large files directory |
...
Binary file bugs discovered during skill testing:

### 1. Describe/Expand CLI Parameter Parsing Issue ✅ FIXED

**Fix**: Added remaining args parsing support for describe and expand commands.

**Correct Usage**:
```bash
# describe - needs session-key as first arg
describe --session-key session_key --id summary_id
describe -session-key session_key -id summary_id

# expand - needs session-key as first arg
expand --session-key session_key --summary-ids id1,id2
expand -session-key session_key --summary-ids 1 --query "query" --max-depth 3
```

**Status**: ✅ FIXED (v0.8.2 cli-fix)

### 2. Maintain -op Parameter Ignored ✅ FIXED

**Fix**: Added support for `-op`, `-maint-op` and other parameter formats for maintain command.

**Correct Usage**:
```bash
maintain -op gc                  # single dash + space
maintain --op=vacuum           # double dash + equals
maintain -op=backup           # single dash + equals
maintain -maint-op doctor       # full parameter name
maintain --maint-op=clean     # full parameter name + equals
```

**Status**: ✅ FIXED (v0.8.2 cli-fix)

### 3. Compact Requires LLM Config

**Symptom**: Even passing LLM config via command line, Compact still reports "summarizer not configured".

**Test Record**:
```bash
$ ./bin/clawlcm compact -session-key test:cli:bootstrap -llm-api-key test -llm-model test -llm-provider test
Error: summarizer not configured
```

**Note**: This may be expected behavior (requires valid config), but the error message is not clear enough.

### 4. Grep/Assemble -all Parameter Behavior Inconsistent

**Symptom**: `assemble -all` requires `--session-key`, but logically `-all` should ignore session-key.

**Test Record**:
```bash
$ ./bin/clawlcm assemble -all
Error: --session-key is required     # ⚠️ Logical conflict

$ ./bin/clawlcm grep -all        # Works
$ ./bin/clawlcm grep --all     # Works
```

### 5. Ingest --messages Parameter Behavior

**Symptom**: `--messages` JSON format not recognized, requires `--content` to be used together.

**Test Record**:
```bash
$ ./bin/clawlcm ingest -session-key test:cli:bootstrap -messages '[{"role":"user","content":"test"}]'
Error: --session-key and --content are required
```

### 3. Compact Summarizer Config (Resolved)

**Note**: Compact can now run normally with LLM config in config.json.
...
```json
{
  "llm": {
    ...
    "provider": "openai",
    "apiKey": "",
    "baseURL": "http://192.168.123.200:18869"
  }
}
```
...
## Test Summary (v0.8.2 cli-fix)

| Command | Status | Notes |
|---------|--------|-------|
| bootstrap | ✅ OK | |
| ingest | ✅ OK | -content, -role |
| assemble | ✅ OK | -limit, -token-budget, -include-messages, -mode |
| compact | ✅ OK | LLM config required |
| grep | ✅ OK | -pattern, -session-key, -all |
| describe | ✅ OK | --session-key first |
| expand | ✅ OK | --summary-ids, --query, --max-depth |
| maintain | ✅ OK | -op, --op, -maint-op |
| tui | ✅ OK | Interactive mode available |